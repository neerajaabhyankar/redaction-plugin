...
class CreateRedactions(foo.Operator):
    @property
    def config(self):
        return foo.OperatorConfig(
            name="create_redactions", 
            label="Create Redactions Operator",
    ...
    def execute(self, ctx):
        chosen_redaction_field = ctx.params.get("redaction_field")
        chosen_redaction_filter = ctx.params.get("redaction_filter")
        chosen_redaction_type = ctx.params.get("redaction_type_choices", "bounding_box")
        chosen_redaction_method = ctx.params.get("redaction_method_choices", "mask")
        #TODO(neeraja): why are we recreating?
        ctx.dataset.persistent = True
        create_redactions(
            ctx.dataset,
            redaction_field=chosen_redaction_field,
            redaction_filter=chosen_redaction_filter,
            redaction_type=chosen_redaction_type,
            redaction_method=chosen_redaction_method,
        )
        return {}

def register(p):
    p.register(CreateRedactions)